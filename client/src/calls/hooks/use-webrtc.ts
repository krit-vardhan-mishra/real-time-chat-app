...
const rtcConfig: RTCConfiguration = {
  iceServers: [
    ...
    // Free TURN servers from Metered.ca for relay fallback
    // These help when peer-to-peer fails behind symmetric NATs or firewalls
    {
      urls: "turn:a.relay.metered.ca:80",
      username: "e8dd65c92f6ec05e0a8a5a6e",
      credential: "k/QaGMDtSV+z0RQo",
    },
    {
...
export function useWebRTC(
  ...
  const getMediaWithFallback = useCallback(
    async (
      media: CallMedia
    ): Promise<{ stream: MediaStream; mediaUsed: CallMedia }> => {
      ...
      try {
        ...
        log("getMedia success", {
          ...
          audioTracks: stream
            .getAudioTracks()
            .map((t) => ({ id: t.id, enabled: t.enabled })),
          videoTracks: stream
            .getVideoTracks()
            .map((t) => ({ id: t.id, enabled: t.enabled })),
        });
        ...
        if (
          ...
          log("getMedia error: retrying without video", {
            err: err?.name,
            message: err?.message,
          });
          try {
  ...
  const startCall = useCallback(
    async (toUserId: number, media: CallMedia) => {
      ...
      try {
        ...
        const pc = createPeer();
        const { stream: localStream, mediaUsed } = await getMediaWithFallback(
          media
        );
  ...
  useEffect(() => {
    ...
    const onCallAnswer = async (payload: {
      ...
      if (callState.status === "calling") {
        setCallState((s) =>
          s.status === "calling"
            ? { status: "in-call", peerId: s.peerId, media: s.media }
            : s
        );
      }
    };
    ...
    const onIceCandidate = async (payload: {
      ...
      try {
        ...
        logger.warn("addIceCandidate failed", e);
      }
    };

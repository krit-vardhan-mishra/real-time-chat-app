...
import type { CallMedia, CallState } from "@/calls/hooks/use-webrtc";
...
export default function CallPanel({
  ...
  const logger = createLogger("CallPanel");
  ...
  useEffect(() => {
    ...
    if (local && localStreamRef.current) {
      logger.debug("bind local video srcObject", {
        hasStream: !!localStreamRef.current,
        tracks: localStreamRef.current?.getTracks().length,
      });
      local.srcObject = localStreamRef.current;
      local.onloadedmetadata = () => {
        logger.debug("local video metadata loaded");
        try {
          local.play().catch((e) => logger.warn("local play failed:", e));
        } catch {}
      };
    ...
    if (remote && remoteStreamRef.current) {
      logger.debug("bind remote video srcObject", {
      ...
      remote.onloadedmetadata = () => {
        logger.debug("remote video metadata loaded");
        try {
          remote.play().catch((e) => logger.warn("remote play failed:", e));
        } catch {}
      };
    ...
    const handleAddTrack = (ev: Event) => {
      logger.debug("remote stream addtrack", {
  ...
  }, [visible, callState.status, localStreamRef, remoteStreamRef]);

  useEffect(() => {
    if (callState.status === "calling") {
      setCallerName(currentUserName);
    } else if (callState.status === "ringing") {
      setCallerName(otherUser?.name);
    }
  }, [callState.status, currentUserName, otherUser?.name]);

  // After all hooks are declared, we can return early based on visibility
      ...
      <div className="w-full max-w-[95vw] sm:max-w-3xl bg-[#0D1117] border border-[#30363D] rounded-xl overflow-hidden shadow-2xl">
        ...
        <div className="p-2 sm:p-3 border-b border-[#30363D] flex items-center justify-between">
          <div className="text-[#C9D1D9] min-w-0 flex-1">
            <div className="font-semibold text-base sm:text-lg truncate">{callerName || "Unknown User"}</div>
            <div className="text-xs sm:text-sm text-gray-400">
              {isRinging && <span>Incoming {media?.video ? "video" : "voice"} call...</span>}
              {isCalling && <span>Calling... ({media?.video ? "video" : "voice"})</span>}
              {inCall && <span className="text-green-500">{media?.video ? "Video call" : "Voice call"} • Connected</span>}
        ...
        <div className="p-2 sm:p-4 flex items-center justify-center gap-2 sm:gap-4 border-t border-[#30363D]">
          {isRinging && (
            <>
              <Button 
                onClick={() => { logger.debug("Accept clicked"); onAccept?.(); }} 
              ...
              <Button 
                variant="destructive" 
                onClick={() => { logger.debug("Reject clicked"); onReject?.(); }}
            ...
            <>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  logger.debug("toggle audio", { enabled: next });
                }}
              ...
              {media?.video && (
                <Button
                  ...
                  onClick={() => {
                    ...
                    logger.debug("toggle video", { enabled: next });
                  }}
                  className="text-gray-300 hover:bg-[#30363D] h-9 w-9 sm:h-10 sm:w-10"
              ...
              <Button 
                variant="destructive" 
                onClick={() => { logger.debug("End clicked"); onEnd?.(); }} 

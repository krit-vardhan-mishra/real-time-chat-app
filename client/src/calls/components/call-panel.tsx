  ...
  useEffect(() => {
    ...
    if (local && localStreamRef.current) {
      ...
      local.onloadedmetadata = () => {
        ...
        try {
          local.play().catch((e) => console.warn("local play failed:", e));
        } catch {}
      };
    ...
    if (remote && remoteStreamRef.current) {
      ...
      remote.onloadedmetadata = () => {
        ...
        try {
          remote.play().catch((e) => console.warn("remote play failed:", e));
        } catch {}
      };
      ...
      <div className="w-full max-w-[95vw] sm:max-w-3xl bg-[#0D1117] border border-[#30363D] rounded-xl overflow-hidden shadow-2xl">
        ...
        <div className="p-2 sm:p-4 flex items-center justify-center gap-2 sm:gap-4 border-t border-[#30363D]">
          {isRinging && (
            <>
              <Button 
                onClick={() => { console.debug("[CallPanel] Accept clicked"); onAccept?.(); }} 
              ...
              <Button 
                variant="destructive" 
                onClick={() => { console.debug("[CallPanel] Reject clicked"); onReject?.(); }}
          ...
          {(isCalling || inCall) && (
            <>
              ...
              <Button 
                variant="destructive" 
                onClick={() => { console.debug("[CallPanel] End clicked"); onEnd?.(); }} 
